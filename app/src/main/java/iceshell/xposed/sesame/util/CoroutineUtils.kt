...
object CoroutineUtils {
    ...
    suspend fun delayCompat(millis: Long) {
        try {
            ...
            Logger.printStackTrace("协程延迟异常", e)
            // 如果协程延迟失败，降级到线程休眠
            Thread.sleep(millis)
        }
    }
    ...
    fun sleepCompat(millis: Long) {
        try {
            ...
            try {
                ...
                Logger.runtime("CoroutineUtils", "延迟被中断: ${ie.message}")
            }
        }
    }
    ...
    fun runOnDispatcher(
        ...
        return CoroutineScope(dispatcher + SupervisorJob()).launch {
            try {
                ...
                Logger.printStackTrace("协程执行异常", e)
            }
        }
    }
    ...
    fun <T> runBlockingSafe(
        ...
        return try {
            ...
            Logger.error("CoroutineUtils", "协程执行超时: ${timeout}ms")
            null
        } catch (e: Exception) {
            Logger.printStackTrace("协程同步执行异常", e)
            null
        }
    }
}
